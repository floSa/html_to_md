...
# Nom de l'extra déclaré dans pyproject.toml qui apporte les formats
# bureautiques. Le message d'erreur doit pouvoir se copier tel quel.
DOCS_EXTRA = "docs"


def _missing_dependency() -> ImportError:
    return ImportError(
        "dépendance manquante pour ce format — installer avec "
        f"`uv sync --extra {DOCS_EXTRA}`"
    )
